...
{
	if (m_nPipeLineIndex == 0) { // 기본 파이프라인
	...
	else if (m_nPipeLineIndex == 1) { // 그림자맵 생성 파이프 라인
...
{
	if (m_nPipeLineIndex == STANDARD_OUT_LINE || m_nPipeLineIndex == STANDARD_OUT_LINE_MASK)
	{
		UINT nInputElementDescs = 5;
		D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
	...
	else if(m_nPipeLineIndex == INSTANCE_OUT_LINE || m_nPipeLineIndex == INSTANCE_OUT_LINE_MASK)
	{
		UINT nInputElementDescs = 9;
		D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
	...
	else if(m_nPipeLineIndex == SKINNING_OUT_LINE || m_nPipeLineIndex == SKINNING_OUT_LINE_MASK)
	{
		UINT nInputElementDescs = 7;
		D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_DEPTH_STENCIL_DESC COutLineShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	if(m_nPipeLineIndex == STANDARD_OUT_LINE_MASK || m_nPipeLineIndex == SKINNING_OUT_LINE_MASK || m_nPipeLineIndex == INSTANCE_OUT_LINE_MASK)
	{
		d3dDepthStencilDesc.DepthEnable = TRUE;
