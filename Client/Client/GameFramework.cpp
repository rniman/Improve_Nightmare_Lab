...
{
	if (m_d3dFramework_info_Resource && m_cbFramework_info)
	{
...
void CGameFramework::CreateEntryWindow(HWND hWnd)
{
	...
	m_hIPAddressEdit = CreateWindowEx(
		...
		FRAME_BUFFER_WIDTH / 2 - nIPEditWidth / 2, FRAME_BUFFER_HEIGHT / 2, nIPEditWidth,
		nIPEditHeight,
		hWnd,
	...
	SendMessage(m_hIPAddressEdit, WM_SETFONT, (WPARAM)m_hEntryFont, TRUE);


	int nIPCaptionWidth = 400;
	int nIPCaptionHeight = 40;
	m_hIPAddressLabel = CreateWindowEx(
		0,
		L"STATIC",
		L"IP ADDRESS",
		WS_CHILD | WS_VISIBLE | SS_CENTER,
		FRAME_BUFFER_WIDTH / 2 - nIPCaptionWidth / 2, FRAME_BUFFER_HEIGHT / 2 - nIPCaptionHeight,
		nIPCaptionWidth, nIPCaptionHeight,
		hWnd,
		(HMENU)EDIT_INPUT_ADDRESS_ID,
		(HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE),
		NULL
	...
	SendMessage(m_hIPAddressLabel, WM_SETFONT, (WPARAM)m_hEntryFont, TRUE);
}

void CGameFramework::OnDestroyEntryWindow()
{
	if (m_hConnectButton && ::IsWindow(m_hConnectButton))
	{
		::DestroyWindow(m_hConnectButton);
	}
	if (m_hIPAddressEdit && ::IsWindow(m_hIPAddressEdit))
	{
		::DestroyWindow(m_hIPAddressEdit);
	}
	if (m_hIPAddressLabel && ::IsWindow(m_hIPAddressLabel))
	{
		::DestroyWindow(m_hIPAddressLabel);
	}
	m_hConnectButton = nullptr;
	m_hIPAddressEdit = nullptr;
	m_hIPAddressLabel = nullptr;

	if (m_hEntryFont)
	{
		::DeleteObject(m_hEntryFont);
		m_hEntryFont = nullptr;
	}
}
