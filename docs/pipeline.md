...
- 입력 송신은 렌더 루프 안에서 최대 60 Hz로 제한하며, 상태 복제는 입력 수신 횟수와 분리된 최대 60 Hz 주기를 사용한다.
- 현재 `UPDATE_DATA`는 최대 5명분의 고정 크기 상태 배열을 각 연결에 전송한다.
...
60 Hz로 제한했다. 2026-08-19 Release 1인 접속에서 `KEYS_BUFFER`는 48 packet/s,
`UPDATE_DATA`는 60 packet/s와 642,060 byte/s로 측정됐다. 큐는 0 byte/0 packet이었고,
문 열기/닫기 이벤트는 상태 복제와 별도로 즉시 전송됐다. 서버 복제는 입력 빈도와 분리됐지만
클라이언트 입력은 렌더 루프에서 호출되므로 60 FPS 미만에서는 프레임 빈도를 따른다.
다중 접속에서도 상태 복제와 큐가 설정 범위에 유지되는지를 다음 회귀 기준으로 사용한다. 구체적인 실행 순서는
