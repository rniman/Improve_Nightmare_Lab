...
1. 프레임워크 소유 공통 Timer를 Tick하고 누적 시간을 캐시한다. 각 Player에 경과·누적 시간을 전달한 뒤 사운드 시스템을 갱신한다.
...
소켓 메시지 진입 시 TCPClient가 프레임워크의 누적 시간을 멤버에 저장하고 내부 처리에서 사용한다. 입력 처리는 시간 인자 없이 Player의 상태를 사용한다. 정지·재개 또는 씬 재생성으로
타이머 상태가 바뀌어도 누적 시간 캐시는 다음 Tick까지 유지하며, 새 Player에도 현재 값을 전달한다.
발자국·공격 궤적의 애니메이션 콜백은 Player에 전달된 시간을 효과 생성 함수에 넘긴다.
피격 화면과 궤적의 시간 차감은 기존 렌더 호출 위치를 유지한다.
...
실행도 확인했다. 이후 Release 로컬 3인 접속까지 실행을 확인했다. partial send/recv와
`WSAEWOULDBLOCK`을 의도적으로 발생시키는 네트워크 부하 테스트 및 5인 장시간 접속
검증은 현재 보류하며, 큐 증가·주기 불안정·연결 문제가 관찰될 때 재개한다.
`ServerWorldBuilder` 분리와 후속 인덱스 검증 변경은 대상 구성 빌드와 서버 시작·월드 생성·
게임 시작 흐름을 실행 확인했다. `ServerScene.bin`이 없는 작업 디렉터리에서 실행했을 때
파일 열기 오류가 출력되고, 월드 구성 실패가 호출부까지 전달되어 리슨 소켓을 열기 전에
서버 프로세스가 종료되는 것도 확인했다.
