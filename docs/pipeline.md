...
현재 구현은 일부 요청에서 `FD_READ` 처리 후 `FD_WRITE` 처리로 이어져 최신 상태를
응답하는 구조를 사용한다. 또한 애플리케이션이 송신을 시작하기 위해 인위적인
`FD_WRITE` 메시지를 게시하는 경로가 있다. 따라서 리팩터링 시에는 다음 두 의미를
구분해야 한다.

1. 애플리케이션에 새로 보낼 패킷이 생김
2. `WOULDBLOCK` 이후 운영체제가 소켓 쓰기 가능 상태를 알림
