...
이 문서는 신규 코드와 점진적 리팩터링의 기준입니다.
기존 레거시 코드를 한 번에 전체 컨벤션으로 변환하지 않습니다.

- 새로 작성하는 코드와 새로 분리되는 모듈에는 본 컨벤션을 우선 적용합니다.
- 기존 파일은 주변 스타일과 동작 보존을 우선합니다.
- 버그 수정, 기능 수정, 리팩터링으로 실제로 검토하는 파일/함수부터 컨벤션을 점진적으로 적용합니다.
- 대량 포맷팅, 전역 네이밍 변경, public API 변경은 기능 수정과 섞지 않고 별도 작업으로 수행합니다.
- 포맷 변경과 동작 변경은 가능한 한 별도 커밋으로 분리합니다.
...
15. [레거시 코드 적용 기준](#15-레거시-코드-적용-기준)
16. [안티패턴](#16-안티패턴)
...
- 기존 레거시 매크로 상수는 별도 리팩터링 작업에서 점진적으로 `constexpr` 또는 `enum class`로 전환합니다.
...
- 헤더에서 완전한 정의가 필요한 타입은 해당 타입을 제공하는 헤더를 직접 include
- PCH 또는 다른 헤더의 간접 include에 의존하지 않음
- PCH는 컴파일 속도 최적화 용도로만 사용하며 코드 의존성을 대신하지 않음
- 상속하는 부모 클래스처럼 완전한 정의가 필요한 타입의 헤더는 생략하지 않음
- 기존 레거시 헤더는 실제로 수정하는 모듈부터 점진적으로 적용
...
**핵심**: 10줄 이상 시 그룹 빈 줄 + 주석, 그룹 내 알파벳 순, 완전한 정의가 필요하면 직접 include하고 PCH나 간접 include에 의존하지 않음
...
기존 코드는 수정하는 범위 안에서만 컨벤션을 적용합니다.
파일 전체 포맷팅, 전체 네이밍 변경, 디렉토리 이동은 별도 작업으로 분리합니다.

### 네이밍

신규 코드에는 본 문서의 네이밍 규칙을 적용합니다.
기존 코드의 `m_nValue`, `m_pObject`, `g_collisionManager` 같은 레거시 네이밍은
해당 모듈을 리팩터링할 때 의미 단위로 변경합니다.

### 포인터와 null

새 코드에는 `nullptr`을 사용합니다.
기존 `NULL`, raw pointer, 수동 `new/delete`는 수정 중인 함수나 명확한 소유권 개선 범위에서 점진적으로 정리합니다.

### 매크로

새 상수는 `constexpr` 또는 `enum class`를 우선합니다.
기존 `GlobalDefine.h` 계열 매크로는 동작 영향 범위가 넓으므로 별도 리팩터링으로 전환합니다.

### 커밋 분리

다음 변경은 가능한 한 서로 다른 커밋으로 분리합니다.

- 인코딩/라인엔딩/들여쓰기 같은 포맷 변경
- 의미 없는 주석 제거, 주석 인코딩 복구
- 버그 수정
- 함수 분리나 책임 이동 같은 리팩터링
- public API 이름 변경
