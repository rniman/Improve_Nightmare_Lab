...
6. **렌더 프레임과 상태 복제 주기의 결합**
   - 클라이언트는 인게임 프레임마다 입력 패킷을 요청하고, 서버는 완성된 클라이언트 패킷을 처리할 때마다 일반 상태 응답을 생성한다. 따라서 입력과 `UPDATE_DATA` 빈도가 클라이언트 렌더 FPS를 따른다.
   - 서버는 각 응답에서 최대 5명분의 고정 크기 `SC_UPDATE_INFO` 배열을 직렬화하므로, 프레임 빈도가 높을수록 큰 스냅샷이 반복 전송된다.
...
6. **입력·복제 주기와 패킷 크기**
   - 렌더 FPS, 클라이언트 입력 송신, 서버 상태 응답이 직접 연결되어 있다. wire 포맷을 변경하기 전에 입력과 상태 복제를 각각 고정 주기로 분리해야 한다.
